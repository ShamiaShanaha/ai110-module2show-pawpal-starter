from pawpal_system import Owner, Pet, Task, Priority, Scheduler, TimeSlot, Frequency  
...
#  tasks added intentionally out of time order to test sort_by_time()
...
#  Whiskers also has a MORNING task — creates a time conflict with Buddy's morning tasks
...
#  sort all tasks by time slot to verify ordering
print("\n=== All Tasks Sorted by Time of Day ===")
for task in scheduler.sort_by_time():
    print(f"  {task}")

# filter by pet name only
print("\n=== Buddy's Tasks Only ===")
for task in scheduler.filter_tasks(pet_name="Buddy"):
    print(f"  {task}")

# filter by status only (all pending)
print("\n=== All Pending Tasks (Every Pet) ===")
for task in scheduler.filter_tasks(completed=False):
    print(f"  {task}")

# filter by both pet name and status
